...
int __rte_cold mqnic_dev_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id);
int __rte_cold mqnic_dev_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id);
void __rte_cold mqnic_dev_rx_queue_release(void *rxq);
static int __rte_cold mqnic_dev_tx_queue_setup(struct rte_eth_dev *dev,
											   uint16_t queue_idx,
											   uint16_t nb_desc,
											   unsigned int socket_id,
											   const struct rte_eth_txconf *tx_conf);
int __rte_cold mqnic_dev_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id);
int __rte_cold mqnic_dev_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id);
void __rte_cold mqnic_dev_tx_queue_release(void *txq);
static int mqnic_dev_start(struct rte_eth_dev *dev);
static int mqnic_dev_stop(struct rte_eth_dev *dev);
static int mqnic_dev_link_update(__rte_unused struct rte_eth_dev *dev, __rte_unused int wait_to_complete);
static int mqnic_promiscuous_enable(__rte_unused struct rte_eth_dev *dev);
static int mqnic_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats);
...
mqnic_dev_close(struct rte_eth_dev *dev)
{
	...
	{
		if (interface->rx_ring[i])
			mqnic_dev_rx_queue_release(interface->rx_ring[i]);
	}
	for (uint64_t i = 0; i < dev->data->nb_tx_queues; ++i)
	{
		if (interface->tx_ring[i])
			mqnic_dev_tx_queue_release(interface->tx_ring[i]);
	}
...
mqnic_common_probe(struct mqnic_dev *mqnic)
{
	...
	PMD_INIT_LOG(NOTICE, "Board version: %d.%d.%d.%d", mqnic->board_ver >> 24, (mqnic->board_ver >> 16) & 0xff,
...
mqnic_dev_info_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info)
{
	...
	dev_info->speed_capa = ETH_SPEED_NUM_10G;
						 ...
						 struct rte_mempool *mp)
{
	...
	ret = mqnic_create_cq_ring(interface, &interface->rx_cpl_ring[queue_idx], &priv->rx_cpl_ring[queue_idx], queue_idx,
							   interface->hw_addr + interface->rx_cpl_queue_offset +
								   queue_idx * interface->rx_cpl_queue_stride);
	ret = mqnic_create_rx_ring(interface, &interface->rx_ring[queue_idx], &priv->rx_ring[queue_idx], queue_idx,
 ...
 */
void __rte_cold
mqnic_dev_rx_queue_release(void *rxq)
{
	PMD_INIT_FUNC_TRACE();
	struct mqnic_ring *ring = (struct mqnic_ring *)rxq;
	struct mqnic_priv *priv = MQNIC_DEV_PRIVATE(ring->dev->data->dev_private);
	PMD_INIT_LOG(NOTICE, "RX Completion Queue %d released", ring->cpl_index);
	mqnic_destroy_cq_ring(ring->dev, priv->rx_cpl_ring[ring->cpl_index], false);
	PMD_INIT_LOG(NOTICE, "RX Queue %d released", ring->index);
	mqnic_destroy_rx_ring(ring->dev, ring);
}
						 ...
						 const struct rte_eth_txconf *tx_conf)
{
	...
	PMD_INIT_FUNC_TRACE();
	PMD_INIT_LOG(NOTICE, "params: queue: %d, nb_desc: %d, socket_id: %d", queue_idx, nb_desc, socket_id);
	ret = mqnic_create_cq_ring(interface, &interface->tx_cpl_ring[queue_idx], &priv->tx_cpl_ring[queue_idx], queue_idx,
 ...
 */
void __rte_cold
mqnic_dev_tx_queue_release(void *txq)
{
	PMD_INIT_FUNC_TRACE();
	struct mqnic_ring *ring = (struct mqnic_ring *)txq;
	struct mqnic_priv *priv = MQNIC_DEV_PRIVATE(ring->dev->data->dev_private);

	PMD_INIT_LOG(NOTICE, "TX Completion Queue %d released", ring->cpl_index);
	mqnic_destroy_cq_ring(ring->dev, priv->tx_cpl_ring[ring->cpl_index], true);
	PMD_INIT_LOG(NOTICE, "TX Queue %d released", ring->index);
	mqnic_destroy_tx_ring(ring->dev, ring);
}
