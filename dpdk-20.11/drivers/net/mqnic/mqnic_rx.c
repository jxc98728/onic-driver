...
int mqnic_alloc_rx_ring(struct rte_eth_dev *dev, struct mqnic_ring *rxq, int size, int stride)
{
...
void mqnic_destroy_rx_ring(struct rte_eth_dev *dev, struct mqnic_ring *ring)
{
	PMD_INIT_FUNC_TRACE();
	if (ring != NULL)
	{
...
int mqnic_prepare_rx_desc(__rte_unused struct rte_eth_dev *dev, struct mqnic_ring *ring, int index)
{
	struct mqnic_rx_info *rx_info = &ring->rx_info[index];
